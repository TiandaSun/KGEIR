...
def retrieve_relevant_paragraphs_direct(llm, context, question, k=10):
...
def simple_answer_question(llm, question, paragraphs):
    ...
    ## Instructions
    Please answer the question directly based on the information provided above and think step by step.
...
def vanilla_baseline(example):
    ...
    while True:
        if len(relevant_paragraphs) > 1:
            ...
            print("No relevant paragraphs found. trying again...")
            relevant_paragraphs = retrieve_relevant_paragraphs_direct(llm, context, question)
            if len(relevant_paragraphs) <= 1:
                print("No relevant paragraphs found. Exiting.")
                break
...
for index, row in dataset.iterrows():
    try:
        ...
        evaluation_pd.to_csv('evaluation_results.csv', index=False)
